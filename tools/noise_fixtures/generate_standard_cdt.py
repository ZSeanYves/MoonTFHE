...
GENERATOR_VERSION = "moontfhe-cdt-u192-v4-integer-literals"
PARAMETERS = (
...
Q32 = 1 << 32
# Keep generated source below compiler memory limits. The generated values are
# integer literals; unlike the old representation they are never parsed at
# runtime.
CHUNK_ENTRIES = 4096
TAIL_SIGMA = 16
...
def render(root: Path) -> tuple[str, dict[str, str], str]:
    ...
    for name, parameter_id, sigma_q32 in PARAMETERS:
        ...
        for chunk_index, chunk in enumerate(chunks):
            ...
            payload_lines = [f"    0x{value:016X}UL," for value in words]
...
def main() -> int:
    ...
    if args.check:
        ...
        def normalized(value: str) -> str:
            # moon fmt intentionally repacks large generated array literals;
            # compare tokens rather than requiring one formatter layout.
            return "".join(value.split())

        stale = any(
            not path.exists()
            or (normalized(path.read_text()) if path != metadata_path else path.read_text())
            != (normalized(text) if path != metadata_path else text)
