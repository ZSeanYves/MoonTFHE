...
目前可维护基线包含 Torus32 运算、朴素负循环多项式、LWE/TLWE/TRLWE 加解密、旧版密钥切换、TRGSW 外积、样本提取，以及一个确定性的实验 LWE 门面。

以下能力被明确排除在稳定公开 API 之外：

- 可编程自举和 NAND/AND/OR 布尔门；
- 旧自举测试使用的明文 oracle；
- 110 位或 128 位安全声明；
- 密码学安全的密钥生成。

旧 oracle 现在只存在于 `oracle_wbtest.mbt`，它会读取旧结构中的秘密字段，只能作为测试参考。当前 `BootstrappingKey` 仍包含这些字段，将在 P3 被替换；不得将它序列化或分发为服务端密钥。
